...
namespace VSI.EDGEAXConnector.MagentoAdapter.Controllers
{
    ...
    public class CustomerController : BaseController, ICustomerController
    {
        ...
        public List<EcomcustomerCustomerEntity> GetCustomer()
        {
            ...
            //string sourceDir = ConfigurationHelper.ECommerceCustomerCSVPath;
            //string localDir = ConfigurationHelper.ConnectorCustomerCSVPath;
        ...
        public void UpdateCustomer(List<EcomcustomerCustomerEntity> customer)
        {
            ...
            try
            {
                ...
                using (CsvWriter writer = new CsvWriter(new StreamWriter(fileNameCustomer)))
                {
                ...
                //string[] files = Directory.GetFiles(ConfigurationHelper.CustomerOutputPathLocal);
                //List<string> failedFiles = new List<string>();
                //SFTPlib.SFTPManager.UploadFiles(files, ConfigurationHelper.CustomerUploadPathFTP, out failedFiles);
                //foreach (var f in files)
                //{
                //    if (!failedFiles.Contains(f))
                //        FileHelper.MoveFileToLocalFolder(f, "Uploaded", ConfigurationHelper.CustomerOutputPathLocal);
                //}
                #endregion
        ...
        //This function has old method logic in which we are generating templates with string implementation
        /*
        public void UpdateCustomerViaXML(List<EcomcustomerCustomerEntity> customers)
        {
            try
            {
                ...
                {
                    string fileNameCustomer = Path.Combine(ConfigurationHelper.CustomerOutputPathLocal, "Updated Customer-" + cus.customer_id + "-" + DateTime.UtcNow.ToString("yyyyMMddhhmm") + ".xml");

                    string strMainClassName = cus.GetType().Name;

                    string customerXmlTemplate = Path.Combine(ConfigurationHelper.XmlPath, "ECom." + strMainClassName + ".xml");
                    if (File.Exists(customerXmlTemplate))
                    {
                        var doc = File.ReadAllText(customerXmlTemplate);

                        PropertyInfo[] cusProperties = cus.GetType().GetProperties().OrderBy(p => p.Name).ToArray();

                        foreach (var prop in cusProperties)
                        {
                            //Handling Collection/List type properties
                            if (prop.PropertyType.IsGenericType && prop.PropertyType.GetGenericTypeDefinition() == typeof(List<>))
                            {
                                string strAttribute = "<" + prop.Name + ">\r\n";

                                string strListClassName = prop.PropertyType.GetGenericArguments()[0].Name;

                                string xmlTemplate = Path.Combine(ConfigurationHelper.XmlPath, "ECom." + strListClassName + ".xml");
                                if (File.Exists(xmlTemplate))
                                {
                                    var docXml = File.ReadAllText(xmlTemplate);

                                    var propValue = cus.GetType().GetProperty(prop.Name).GetValue(cus);
                                    if (propValue != null)
                                    {
                                        IEnumerable lstProp = propValue as IEnumerable;
                                        if (lstProp != null)
                                        {
                                            foreach (var pro in lstProp)
                                            {
                                                PropertyInfo[] lstObjectProperties = pro.GetType().GetProperties().OrderBy(p => p.Name).ToArray();
                                                foreach (var proObj in lstObjectProperties)
                                                {
                                                    var pValue = pro.GetType().GetProperty(proObj.Name).GetValue(pro);
                                                    if (pValue != null)
                                                    {
                                                        docXml = docXml.Replace("[" + strListClassName + "." + proObj.Name + "]", pValue.ToString());
                                                    }
                                                    else
                                                    {
                                                        docXml = docXml.Replace("[" + strListClassName + "." + proObj.Name + "]", "");
                                                    }
                                                }
                                ...
                                strAttribute += "</" + prop.Name + ">";
                                
                                doc = doc.Replace("<" + prop.Name + " />", strAttribute);
                            }
                            //Handling Runtime class properties
                            else if(prop.GetType().Name.Equals("RuntimePropertyInfo"))
                            {
                                string strClassName = prop.PropertyType.GetGenericArguments()[0].Name;

                                string xmlTemplate = Path.Combine(ConfigurationHelper.XmlPath, "ECom." + strClassName + ".xml");
                                ...
                                if (File.Exists(xmlTemplate))
                                {
                                    ...
                                    if (propValue != null)
                                    {
                                        PropertyInfo[] lstObjectProperties = propValue.GetType().GetProperties().OrderBy(p => p.Name).ToArray();
                                        foreach (var proObj in lstObjectProperties)
                                        {
                                            var pValue = propValue.GetType().GetProperty(proObj.Name).GetValue(propValue);
                                            if (pValue != null)
                                            {
                                                docXml = docXml.Replace("[" + strClassName + "." + proObj.Name + "]", pValue.ToString());
                                            }
                                            else
                                            {
                                                docXml = docXml.Replace("[" + strClassName + "." + proObj.Name + "]", "");
                                            }
                                        }
                                    }
                                    strAttribute += docXml + "\r\n";
                            ...
                            {
                                var propertyValue = cus.GetType().GetProperty(prop.Name).GetValue(cus);
                                if (propertyValue != null)
                                {
                                    doc = doc.Replace("[" + strMainClassName + "." + prop.Name + "]", propertyValue.ToString());
                                }
                                else
                                {
                                    doc = doc.Replace("[" + strMainClassName + "." + prop.Name + "]", "");
                                }
                            }
                        }

                        var xmlDoc = XDocument.Parse(doc);
                        xmlDoc.Save(fileNameCustomer);
                    }
                }
        ...
        */
        ...
        //This function has new method logic in which we are generating templates with XML nodes 
        public void UpdateCustomerViaXML(List<EcomcustomerCustomerEntity> customers)
        {
            try
            {
                foreach (var cus in customers)
                {
                    ...
                    xmlHelper.GenerateXmlUsingTemplate(fileNameCustomer, ConfigurationHelper.GetDirectory(configurationHelper.GetSetting(CUSTOMER.Local_Output_Path)), XmlTemplateHelper.XmlSourceDirection.CREATE, cus);
                }
        ...
        public void UpdateAddress(List<EcomcustomerAddressEntityItem> customer)
        {
            ...
            try
            {
                ...
                using (CsvWriter writer = new CsvWriter(new StreamWriter(fileNameAddress)))
                {
                    List<string> headerRow = new List<string>();
                ...
                //string[] files = Directory.GetFiles(ConfigurationHelper.CustomerAddressOutputPathLocal);
                //List<string> failedFiles = new List<string>();
                //SFTPlib.SFTPManager.UploadFiles(files, ConfigurationHelper.CustomerAddressUploadPathFTP, out failedFiles);
                //foreach (var f in files)
                //{
                //    if (!failedFiles.Contains(f))
                //    {
                //        FileHelper.MoveFileToLocalFolder(f, "Uploaded", ConfigurationHelper.CustomerAddressOutputPathLocal);
                //    }
                //}
        ...
        private EcomcustomerCustomerEntity CustomerCSVToEcommerceCustomer(CustomerCSV csv)
        {
            ...
            //ecom.rp_token = Convert.ToBoolean(csv.confirmation);
            //ecom.rp_token_created_at = Convert.ToBoolean(csv.confirmation);
            //  ecom.store_id = Convert.ToInt32(csv.store_id);
            ...
            //EcomcustomerAddressEntityItem address = CustomerCSVToEcommerceAddress(csv);
            //ecom.addresses.Add(address);
        ...
        private CustomerCSV2 ConvertAddress(EcomcustomerAddressEntityItem CustAddress)
        {
            try
            {
                ...
                /*
                //We will send the PK of Integration Key entry to Magento as edgeaxintegration key, so that on return, we could avoid the effort required to resolve it for expire RecIds.
                //First call to get Integration Key should always return the an Integration Key object.
                var ik = IntegrationManager.GetComKey(VSI.EDGEAXConnector.Common.Enums.Entities.CustomerAddress, integKey);
                if (ik == null)
                {
                    string recordInfo = string.Format("Csutomer Id: {0} \n Address Id: {1}\n City: {2} \n Telephone: {3} \n Street: {4} \n State: {5} \n ERP Address Id: {6}",
                        CustAddress.customer_address_id, CustAddress.city, CustAddress.telephone, CustAddress.street, CustAddress.region, CustAddress.edgeaxintegrationkey);
                    CustomLogger.LogTraceInfo("Critical! Integration key is going to created inside Ecomm Controller while preparing Address CSV. I should be created in AX controller while fetching data from AX. Please investigate." + Environment.NewLine + " RECORD INFO" + Environment.NewLine + recordInfo);
                    IntegrationManager.CreateIntegrationKey(VSI.EDGEAXConnector.Common.Enums.Entities.CustomerAddress, integKey, addressId);
                    ik = IntegrationManager.GetComKey(VSI.EDGEAXConnector.Common.Enums.Entities.CustomerAddress, integKey);
                }
                AddressCSV._edgeaxintegrationkey = ik.Id.ToString();
                */
