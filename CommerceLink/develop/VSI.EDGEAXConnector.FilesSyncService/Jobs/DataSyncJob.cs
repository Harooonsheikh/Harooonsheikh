...
namespace VSI.EDGEAXConnector.FilesSyncService.Jobs
{
    class DataSyncJob: IJob
    {
        public void Execute(IJobExecutionContext context)
        {
            CustomLogger.LogSyncTrace(0,0,"", "DataSyncJob.Execute", $"Entered Method");
            ...
            catch (Exception ex)
            {
                ...
                manager.UpdateJobStatus(jobSchedule, SynchJobStatus.Available);
                CustomLogger.LogException(ex, manager.GetSchedule().StoreId, manager.GetSchedule().CreatedBy); ;
