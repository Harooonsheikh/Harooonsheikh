...
{
    class QuantityDiscountManager : IJobManager
    {
        public static readonly string IDENTIFIER = "UploadQuantityDiscountSync";
        ...
        public Job GetJob()
        {
            ...
            Job job = jobRepo.GetJob((int)Common.Enums.SyncJobs.UploadQuantityDiscountSync);
            return job;
        }
        ...
        public bool Sync()
        {
            try
            {
                ...
                string sourceDirUpload = this.configurationHelper.GetDirectory(
                    configurationHelper.GetSetting(QUANTITYDISCOUNT.Local_Output_Path));
                string localDirUpload = configurationHelper.GetSetting(QUANTITYDISCOUNT.Remote_Path);
            ...
            {
                CustomLogger.LogException("SyncQuantityDiscount upload Exception" + Environment.NewLine + Common.CommonUtility.GetExceptionInfo(ex), store.StoreId, store.CreatedBy);
                throw;
            }
        }
        ...
        {
            JobRepository jobRepo = new JobRepository(this.store.StoreKey);
            return jobRepo.GetJobSchedule((int)Common.Enums.SyncJobs.UploadQuantityDiscountSync, this.store.StoreId);
        }
