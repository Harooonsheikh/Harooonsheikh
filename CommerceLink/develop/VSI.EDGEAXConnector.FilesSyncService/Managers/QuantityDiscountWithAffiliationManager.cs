...
namespace VSI.EDGEAXConnector.FilesSyncService.Managers
{
    public class QuantityDiscountWithAffiliationManager : IJobManager
    {
        public static readonly string IDENTIFIER = "UploadQuantityDiscountWithAffiliationSync";
        ...
        public Job GetJob()
        {
            ...
            Job job = jobRepo.GetJob((int)Common.Enums.SyncJobs.UploadQuantityDiscountWithAffiliationSync);
            return job;
        }
        ...
        public bool Sync()
        {
            try
            {
                ...
                string sourceDirUpload = this.configurationHelper.GetDirectory(
                    configurationHelper.GetSetting(QUANTITYDISCOUNTWITHAFFILIATION.Local_Output_Path));
                string localDirUpload = configurationHelper.GetSetting(QUANTITYDISCOUNTWITHAFFILIATION.Remote_Path);
            ...
            {
                CustomLogger.LogException("SyncQuantityDiscountWithAffiliation upload Exception" + Environment.NewLine + Common.CommonUtility.GetExceptionInfo(ex), store.StoreId, store.CreatedBy);
                throw;
            }
        }
        ...
        {
            JobRepository jobRepo = new JobRepository(this.store.StoreKey);
            return jobRepo.GetJobSchedule((int)Common.Enums.SyncJobs.UploadQuantityDiscountWithAffiliationSync, this.store.StoreId);
        }
