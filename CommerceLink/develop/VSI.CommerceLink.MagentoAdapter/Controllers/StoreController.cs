...
namespace VSI.CommerceLink.MagentoAdapter.Controllers
{
    ...
    public class StoreController : BaseController, IStoreController
    {
        ...
        public void CreateStores(List<EcomstoreEntity> stores)
        {
            ...
            try
            {
                string fileName = Path.Combine(configurationHelper.GetSetting(STORE.Local_Output_Path), "Store Inventory" + currentStore.Name + "-" + DateTime.UtcNow.ToString("yyyyMMddhhmmssfff") + ".csv");
        ...
        private void CreateStoreInfoFile(ErpStoreInfo erpStores)
        {
            try
            {
                if (erpStores != null)
                {
                    string fileNameStores = "Stores" + "-" + currentStore.Name + "-" + DateTime.UtcNow.ToString("yyyyMMddhhmmssfff") + ".xml";
                    XmlTemplateHelper xmlHelper = new XmlTemplateHelper(currentStore);
                    xmlHelper.GenerateXmlUsingTemplate(fileNameStores, this.configurationHelper.GetDirectory(configurationHelper.GetSetting(STORE.Local_Output_Path)), XmlTemplateHelper.XmlSourceDirection.CREATE, erpStores);
                }
            }
            catch
            {
