    ...
    public class SalesOrderStatusController : BaseController, ISalesOrderStatusController
    {
        ...
        private ErpOrderStatus ProcessOrderStatusData(List<ErpSalesOrderStatus> ecomOrders)
        {
            ...
            foreach (ErpSalesOrderStatus ord in ecomOrders)
            {
                ...
                //var ecomOrder = base.Service.salesOrderInfo(base.SessionId, ord.ChannelRefId);

                var magentoOrderDetail = base.Api.SalesOrder(ord.ChannelRefId);
                if (magentoOrderDetail!=null && !magentoOrderDetail.status.ToLower().Equals("complete"))
                {
                    var result = base.Api.ChangeOrderStatus(magentoOrderDetail.entity_id, magentoOrderDetail.increment_id, ord.Status.ToLower().Equals("invoiced") ? "complete" : "pending");
                }
