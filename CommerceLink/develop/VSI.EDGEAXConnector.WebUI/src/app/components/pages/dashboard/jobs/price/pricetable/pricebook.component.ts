...
@Component({
    selector: "pricebook-table",
    templateUrl: "pricebook.component.html"
})
export class PriceBookComponent implements OnInit, AfterViewInit {
    @Input() public inputFile: string = "";
    public total: number = null;
    private priceBookOb: Observable<Object> = null;
    public entityList: Array<PriceEntity> = null;
    public dataTable: any = null;
    public searchQuery: string = "";
    public searchResults: Array<PriceEntity> = null;

    constructor(private _pService: PriceService) { }

    ngOnInit() { }
    ngAfterViewInit() {

        this._pService.getRecordCount(this.inputFile).subscribe(
            (number: number) => {
                this.total = number;
    ...
    private setupProductTable(): void {
        ...
        this.priceBookOb = this._pService.getRecords(this.inputFile, 0 * 10, 10);
        ...
        this.dataTable = $("#priceBookTable").mDatatable({
            data: {
                type: "local",
    ...
    private initEvents(): void {
        ...
        var perPage = 10;
        //this._pService.getProductByCatalog(this.selectedCatalogFile, pages * perPage, perPage)
        this.priceBookOb.subscribe(
            (m: string) => {
        ...
        let self: PriceBookComponent = this;
