...
set [XML] = '<Items>
  <item repeat="true" data-source="ErpCatalog~Products" product-id="ErpProduct~ItemId" mode="ErpProduct~Mode">
    ...
    <erp_attributes expression="true"> String.IsNullOrEmpty(sourceObject.Locale) ? ( sourceObject.CustomAttributes != null ? (string.Join(",", sourceObject.CustomAttributes.Select(att=&gt;att.Key + "=" +  (att.Value!=null? att.Value:string.Empty)))) : string.Empty ) : string.Empty </erp_attributes>
    ...
    <cl_split_item expression="true">sourceObject.CustomAttributes != null &amp;&amp; !string.IsNullOrWhiteSpace(sourceObject.CustomAttributes.FirstOrDefault(att =&gt; att.Key == "cl_split_item").Value) ? sourceObject.CustomAttributes.FirstOrDefault(att =&gt; att.Key == "cl_split_item").Value : string.Empty</cl_split_item>
